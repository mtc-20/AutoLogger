...
while(True):
    ...
    #code for finding no. of defects due to fingers
        ...
        if l==1:
            if areacnt<2000:
                cv2.putText(frame,'Put rock for coming in and any other for going out inside the box',(0,50), font, 2, (0,0,255), 3, cv2.LINE_AA)
            else:
                if arearatio<12:
...
if day+'-'+month+'-'+year+'.txt' not in day_glob:
    ...
    #f=open(full_path,"a+")
    if 'in' in entry:
        if '[Check In] Username: '+name in a:
            print('You are already in. Try again with hi-five to go out')
        else:
            g=open(full_path,"a+")
